...
人机共同编辑工作区里的文件，是再常见不过的情景：agent 在推理、改文件、跑命令，你也在同一批文件上动手——删掉或重命名某个文件、改掉调试脚本里的一个函数名、顺手修一处拼写（更不用说格式化器、构建产物、分支切换这些外部进程的改动）。但 agent 对你这些外部操作基本是无感知的。

于是，等它终于撞上这些从未进入上下文的改动时，往往已经措手不及：它可能专门回退一个提交，只为「恢复」你主动删掉的文件；也可能顺着早已过时的上下文一路做下去，直到被你改名的那个函数硬生生拦住。要避免这些坑，本该靠你事无巨细地逐条告知——可这恰恰是对用户的负担。

bright-drift 要解决的，正是这种人机共同编辑中的摩擦。它持续监控工作区，为每个会话维护一份「agent 已经见过什么」的基线（*Agent Knowledge Base*，AKB），并在下一次模型请求前注入一条紧凑、受预算约束的漂移通知——把「发生了什么」交给插件去说，而不是让用户逐条交代：
...
时效语义（与一期的差异，烟雾测试后澄清）：通知是**批处理式**的——一次发言汇总自上次告知以来的全部变更，diff 永远从「agent 最后被告知的版本」算到当时的最新磁盘状态（保存后反复编辑会合并为一条净 diff，不逐中间态上报）。检测侧是实时的（watcher 去抖 300ms 内即渲染完毕），唯一的固有窗口是：**保存后约 1 秒内立刻回车**，最新一笔可能顺延到下一条消息（去抖 + 渲染耗时 + turn 边界投递的叠加）。
